...
async def insert_multiple_users():
    ...
    user1 = User(name="John", email="john@example.com", location=Location(city="Taipei", country="JA"))
    user2 = User(name="Mary", email="mary@example.com", location=Location(city="TaoYuan", country="JA"))
...
async def find_some_users():
    ...
    users: list[User] = await User.find(User.location.country == "JA").sort(-User.name).to_list()

    # noinspection PyUnresolvedReferences
    user_query = User.find(User.location.country == "JA").sort(-User.name)

    async for user in user_query:
        user.password_hash = "a"
        await user.save()

    print("Upgraded security for all users in Japan.")
